...
# Do not remove.
import nas_connector.swift.common.constraints  # noqa

import errno
import os

from nas_connector.swift.common.utils import delete_tracker_object
...
EXCLUDE_DIRS = ('.trashcan')


class NasConnectorInternalClient(InternalClient):

    def __init__(self, conf_path, user_agent, request_tries,
                 allow_modify_pipeline=False, devices=None):
        super(NasConnectorInternalClient, self).__init__(
...
class ObjectExpirer(SwiftObjectExpirer):
    ...
    def __init__(self, conf, logger=None, swift=None):
        ...
        swift = NasConnectorInternalClient(
            conf_path, 'Nas Connector Object Expirer', request_tries,
    ...
    def pop_queue(self, container, obj):
        ...
        container database. In Nas Connector, this method deletes tracker
    ...
    def delete_actual_object(self, actual_obj, timestamp):
        ...
        Don't do this in Nas Connector. NAS shares aren't eventually consistent
