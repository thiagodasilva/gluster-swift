...
from nas_connector.swift.common.fs_utils import do_fstat, do_open, do_close, \
    do_unlink, do_chown, do_fsync, do_fchown, do_stat, do_write, do_read, \
    do_fadvise64, do_rename, do_fdatasync, do_lseek, do_mkdir
from nas_connector.swift.common.utils import read_metadata, write_metadata, \
    validate_object, create_object_metadata, rmobjdir, dir_is_object, \
    get_object_metadata
from nas_connector.swift.common.utils import X_CONTENT_TYPE, \
...
def make_directory(full_path, uid, gid, metadata=None):
    ...
    try:
        ...
        if err.errno == errno.ENOENT:
            ...
            except NasConnectorFileSystemOSError as serr:
        ...
        while True:
            try:
                ...
                if err.errno in (errno.ENOENT, errno.EIO,
                    ...
                    raise NasConnectorFileSystemOSError(
...
class DiskFile(object):
    ...
    nas_connector.common.constrains.check_object_creation() should
    ...
    def open(self, modernize=False, current_time=None):
        ...
        except NasConnectorFileSystemOSError as err:
        ...
        try:
            ...
            if hasattr(err, 'errno') and \
                ...
                # call if file gets deleted by another
    ...
    def get_datafile_metadata(self):
        '''nas connector doesnt have seperate data, meta files '''
        if self._metadata is None:
            raise DiskFileNotOpen()
    ...
    def create(self, size=None):
        ...
        We first optimistically create the temporary file using
        ...
        while True:
            ...
            except NasConnectorFileSystemOSError as gerr:
