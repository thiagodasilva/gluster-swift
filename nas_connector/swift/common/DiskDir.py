...
from nas_connector.swift.common.fs_utils import dir_empty, mkdirs, do_chown, \
    do_exists, do_touch, do_stat
from nas_connector.swift.common.utils import validate_account, validate_container, \
...
from nas_connector.swift.common.exceptions import FileOrDirNotFoundError, \
    NasConnectorFileSystemIOError
from nas_connector.swift.obj.expirer import delete_tracker_object
...
#TODO need to look into removing this
...
class DiskCommon(object):
    ...
    def __init__(self, root, drive, account, logger, pending_timeout=None,
        ...
        global _db_file
        if not _db_file:
            _db_file = os.path.join(manager.RUN_DIR, 'db_file.db')
...
class DiskDir(DiskCommon):
    ...
    :param drive: nas share mount point
    ...
    def list_objects_iter(self, limit, marker, end_marker,
        ...
        for obj in objects:
            ...
            if not metadata or not validate_object(metadata):
                ...
                try:
                    ...
                    if e.errno not in (errno.ENOENT, errno.ESTALE):
                        raise
            if _implicit_dir_objects and metadata \
            ...
            if metadata:
                ...
                if metadata[X_CONTENT_TYPE] == DIR_TYPE:
                    # [tdasilva] - fix to return delimeter as subdir
                    metadata[X_CONTENT_TYPE] = None
    ...
    def get_info(self):
        ...
        """
        if self._dir_exists and _container_update_object_count and \
                self.account != 'gsexpiring':
...
class DiskAccount(DiskCommon):
    ...
    def __init__(self, root, drive, account, logger, **kwargs):
        ...
        # share mount point directly, and the mount has already been checked at
    ...
    def is_status_deleted(self):
        ...
        # and deleted, they exist if a NAS share volume can be mounted. There is
    ...
    def delete_db(self, timestamp):
        ...
        # mappings to share mount points.
        #
    ...
    def get_info(self):
        ...
        :returns: dict with keys: account, created_at, put_timestamp,
                  ...
                  bytes_used, hash, id
        """
        if _account_update_container_count and \
                self.account != 'gsexpiring':
